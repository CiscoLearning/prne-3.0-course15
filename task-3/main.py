...
def generate_device_config(device_info: dict, requirements: str) -> str:
    ...
    Args:
        device_info (dict): Device information from inventory with name and IP details
        requirements (str): Natural language description of desired configuration

    Returns:
        str: Generated Cisco IOS XE configuration commands
    """
...
def main():
    ...
    for device_data in inventory:
        ...
        if generated_config:
            ...
            if config_commands:
                ...
                if apply.lower() == 'y':
                    if deploy_configuration(device_data, config_commands):
                        successful_deployments += 1
                        print(f"✓ Configuration successfully applied to {device_name}")
                    else:
                        print(f"✗ Configuration deployment failed for {device_name}")
                else:
    ...
    print(f"Failed deployments: {total_devices - successful_deployments}")
    print("AI-powered network automation completed")
